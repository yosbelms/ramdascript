...
RamdaScript is a little functional language that lets you write JavaScript in the [Ramda](http://ramdajs.com) way. There are already several functional languages that targets JavaScript. Commonly they are full-featured with rigid philosophy, own architecture and standard libraries. RamdaScript is lean, focused on simplicity by combining the power of [Ramda](http://ramdajs.com) with the elegance of S-Expressions. With full interoperability with the current JavaScript ecosystem.
...
Compile to JavaScript and save as .js file
  ...
  -dst    Route where the resulting JavaScript code wil be saved
  -nowrap Whether do not use CommonJS wrapper
...
Resets and returns the value of a variable. This function is curried.

```
(reset app.state newState)
...
The source code contained between these wrappers will pass through to the resulting JavaScript code as is.
